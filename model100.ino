...
static void tap_copy_action(uint8_t tapCount, KeyAddr key_addr, kaleidoscope::plugin::TapDance::ActionType tapDanceAction) {

 // on a double dot instead insert a dot, space and shift to auto caps the first key pressed
 if (tapCount == 1) {
    Kaleidoscope.hid().keyboard().pressKey(Key_Period);
    Kaleidoscope.hid().keyboard().sendReport();  
    Kaleidoscope.hid().keyboard().releaseKey(Key_Period);
 }
 else if (tapCount == 2) {
    Kaleidoscope.hid().keyboard().pressKey(Key_Period);
    Kaleidoscope.hid().keyboard().pressKey(Key_Space);
    Kaleidoscope.hid().keyboard().sendReport();    
    Kaleidoscope.hid().keyboard().releaseKey(Key_Period);  
    Kaleidoscope.hid().keyboard().releaseKey(Key_Space);
...
void tapDanceAction(uint8_t tap_dance_index, KeyAddr key_addr, uint8_t tap_count,
    ...
    case 1:
      return tapDanceActionKeys(tap_count, tap_dance_action, Key_Escape, Key_Backtick);
    case 2: {
          return tapDanceActionKeys(tap_count, tap_dance_action, Key_V, LGUI(Key_V));
        }
    case 3: {
          return tapDanceActionKeys(tap_count, tap_dance_action, Key_C, LGUI(Key_C));
        }
    case 4: {
          return tapDanceActionKeys(tap_count, tap_dance_action, Key_X, LGUI(Key_X));
        }
    case 5: {
          return tapDanceActionKeys(tap_count, tap_dance_action, Key_Z, LGUI(Key_Z));
        }
    case 6: {
          return tapDanceActionKeys(tap_count, tap_dance_action, Key_A, LGUI(Key_A));
        }
      case 7: {
          return tap_copy_action(tap_count, key_addr, tap_dance_action);
        }
    }
...
void setup() {
  ...
  QUKEYS(
    ...
    kaleidoscope::plugin::Qukey(0, KeyAddr(3, 4), LGUI(Key_V)),
    kaleidoscope::plugin::Qukey(0, KeyAddr(3, 3), LGUI(Key_C)),
    kaleidoscope::plugin::Qukey(0, KeyAddr(3, 2), LGUI(Key_X)),
    kaleidoscope::plugin::Qukey(0, KeyAddr(3, 1), LGUI(Key_Z)),
     
  )
  Qukeys.setHoldTimeout(800);
  Qukeys.setOverlapThreshold(80);
  Qukeys.setMinimumHoldTime(250);
  Qukeys.setMinimumPriorInterval(90);
  Qukeys.setMaxIntervalForTapRepeat(150);
  /*
  AutoShift.enable();
  AutoShift.letterKeys();
